...
import "./3_SeroInterface.sol";

contract test_token is SeroInterface ,Owner { 
    ...
    function transfer(address _to, uint256 _value) public isOwner returns (bool success) {
        uint256 balance = sero_balanceOf(_symbol);
    ...
    function addIssue(uint256 total) public isOwner returns(bool success) {
        require(sero_issueToken(total,_symbol));
...
module.exports = {
  ...
  seroIntereface: { name: '3_SeroInterface.sol', content: seroIntereface },
